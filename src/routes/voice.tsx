  ...
  async function startRecording() {
    try {
      ...
      mr.onstop = async () => {
        ...
        if (chunksRef.current.length === 0 || blob.size < 500) {
          toast.error("Didn't catch that. Hold the mic a moment longer and try again.");
          return;
        }
        await processAudio(blob, type);
      };
      // Timeslice ensures audio chunks are captured reliably across browsers.
      mr.start(250);
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 py-12">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-xs font-medium">
            <Sparkles className="size-3.5 text-primary" /> Voice AI · powered by ElevenLabs + Gemini
          </div>
        ...
        <p className="text-center text-xs text-muted-foreground mt-6">
          Speech is transcribed by ElevenLabs / Deepgram. Replies generated by Gemini.
        </p>
      </div>
    </div>
  );
